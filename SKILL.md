...
- **Re-parse is destructive**: old DB rows are deleted *before* the new file write completes. After any re-parse, verify with `list` + `toc` (chunk count, expected sections present).
