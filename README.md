...
- **Hybrid PDF Document Structuring**: Automatically clusters drawings/graphics, crops them to high-resolution PNGs, redacts noisy overlapping text from the main body, and inserts clean metadata anchors.
- **Tag-Based Catalog Management**: Supports assigning document tags during parsing or post-parsing to maintain a structured global knowledge catalog (`global_catalog.md`).
...
Run the following command in your terminal from the project root to install the package and its dependencies:
```bash
pip install -e .
```
...
### 1. Parse a Document
Parses a PDF/DOCX, extracts layout/chunks/images, assigns optional tags, and stores them.
```bash
doc-structuring parse --file "your-manual.pdf" --tags "Intel Platform,Technical Manual" --output "parse_result.json"
```
...
```bash
doc-structuring search --query "ACPI" --output "search_results.json"
```
...
### 7. Assign or Update Tags
Assigns or updates tags for an already parsed document, and updates the global catalog.
```bash
doc-structuring tag --doc-id 1 --tags "New Tag,Another Tag"
```
- **Stdout Output**:
  `Success: Tags for document ID 1 set to: New Tag, Another Tag`

---

## Database Schema
The SQLite database file `documents.db` contains four tables:
...
- **`document_tags`**:
  - `document_id`: Foreign key cascading on delete.
  - `tag`: The classification tag string.
...
- **混合式圖表結構化**：自動群組化向量圖元與圖片，裁剪渲染成高解析度 PNG 檔案，遮蔽主內文中的重疊「文字沙拉」，並利用 Margin 錨點進行關聯。
- **分類標籤與全域目錄**：支援在解析時設定標籤或事後使用 `tag` 指令更新標籤，全自動維護全域知識庫目錄 (`global_catalog.md`)。
...
### 1. 解析檔案 (Parse)
將 PDF 或 DOCX 長文件切片、提取圖形/區塊圖並存入資料庫與磁碟，可指定分類標籤。
```bash
doc-structuring parse --file "your-manual.pdf" --tags "Intel Platform,Technical Manual" --output "parse_result.json"
```
...
## 資料庫結構
SQLite 資料庫檔案預設為 `documents.db`，由以下四張資料表組成：
...
1. **`ModuleNotFoundError: No module named 'fitz'` / `'docx'`**
   ...
   - **Solution**: Ensure your python environment is active, navigate to the project root, and install the package:
     `pip install -e .`
   - **原因**：目前作用中的 Python 環境尚未安裝必要的解析庫。
   - **解法**：請確保使用正確的 Python 環境，切換至專案根目錄並執行：
     `pip install -e .`
...
3. **Relative Path / Working Directory Issues / 工作路徑混亂**
   ...
   - **Solution**: Always navigate to the **project workspace root** (where `documents.db` and `output/` should reside) before running the CLI.
   - **原因**：於 `scripts/` 資料夾內或任意全域路徑執行指令，會導致資料庫與 Markdown 檔案被建立在錯誤的地方。
   - **解法**：請一律將終端機切換至 **專案工作目錄根路徑**，再行執行 `doc-structuring ...`。
